...
The final report should be submitted separately as:

```text
Trend.pdf
```

Recommended course submission package:

```text
Trend.pdf
neurips_trend_code_final.zip
```

Optional reproducibility resources:

```text
Hugging Face dataset:
https://huggingface.co/datasets/tuandebu/data_mining/tree/main

report_ready_outputs.tar.gz
```

Do not submit the full artifact archive unless explicitly requested, because it may be very large.

---

## 22. Suggested GitHub Repository Description

Use this in the GitHub **About** section:

```text
Review-guided topic mining of NeurIPS 2023–2025 papers using BERTopic, LDA baseline, stability analysis, and review–trend regression. Data hosted on Hugging Face.
```
...
This repository is prepared for COMP4040 final project submission. The final report uses the corrected accepted-corpus framing, model-selection results, topic-trend decomposition, review–trend association analysis, and representative-paper validation.
